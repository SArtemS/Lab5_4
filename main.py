...
# Убрать один из списков, по максимальному элементу
def fib_gen(l):
    while True:
        i = l[-2]
        yield i + l[-1]
...
class FibonacchiLst:
    ...
    def __next__(self): # возвращает следующий по порядку элемент итератора
        ...
        while True:
            try:
                res = fib_instance[self.idx] # получаем очередной элемент из iterable
...
def main():
    x = int(input("Задание 1\nВведите максимальное число элементов ряда Фибоначчи: "))
    fib_list = fib(x)
    print(fib_list)
    user_lst_2 = []
    ...
    y = int(input("Задание 4\nВведите максимальное число элементов ряда Фибоначчи: "))
    fib_gen_list = fib_gen_ver(y)
    print(fib_gen_list)
